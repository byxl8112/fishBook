...
for i in range(iters_num):
    ...
    if i % iter_per_epoch == 0:  # 每经过一次 epoch 记录一次数据（准确率）
        train_acc = network.accuracy(x_train, t_train)  # 算全量训练集准确率，看看模型是不是死记硬背的能力，如果很低说明模型太笨了
        test_acc = network.accuracy(x_test, t_test)  # 算全量测试集准确率，测试集图片没有参与过计算和更新，看有没有泛化能力
